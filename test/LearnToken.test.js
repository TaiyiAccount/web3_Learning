...
describe("TokenSale", function () {
  ...
  it("Should allow buying tokens with ETH", async function () {
    ...
    expect(newTokenBalance).to.equal(initialTokenBalance + ethers.parseEther("100")); // 1 ETH = 100 个完整代币
  });

  it("Should prevent buying when insufficient tokens", async function () {
    // 尝试购买超过可用代币数量（10000个代币可用，尝试购买需要10001个代币）
    await expect(
      tokenSale.connect(buyer).buyTokens({
        value: ethers.parseEther("101") // 101 ETH = 10100 代币，超过可用的10000代币
      })
