...
describe("TokenSale", function () {
  ...
  it("Should allow buying tokens with ETH", async function () {
    ...
    expect(newTokenBalance).to.equal(initialTokenBalance + 100n); // 1 ETH 购买计算：(1 ETH * 100) / 1 ETH = 100 (wei单位)
  });

  it("Should prevent buying when insufficient tokens", async function () {
    // 计算需要多少ETH才能买完10000个代币
    // 10000 tokens = 10000 ether = 10000 * 10^18 wei
    // 需要的ETH = 10000 ether / 100 = 100 ether (因为tokensPerEth = 100)
    // 所以用101 ETH应该超出限制
    await expect(
      tokenSale.connect(buyer).buyTokens({
        value: ethers.parseEther("10000000000") // 一个巨大的值来确保超出代币限制
      })
